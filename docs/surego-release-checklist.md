...
- 参与者路径：详情 -> 报名 -> 参与者中心 -> 订单详情 -> 支付占位 -> 入场凭证。
...
- 验证 A 创建活动、B 报名、A 审核、B 查看凭证、B 模拟支付、A 核销、运营处理举报的完整链路。

## 微信发布合规

- 补齐小程序隐私协议，覆盖用户信息、相册保存、位置、扫码、云存储等能力说明。
- 确认真实支付尚未接入，支付页仍为模拟支付说明，不出现真实扣款承诺。
- 海报页不请求真实小程序码接口，二维码区域仍为占位视觉。
- 地图、朋友圈、聊天、局长主页仍为试运营占位能力，提示文案应清楚且不阻断核心流程。
